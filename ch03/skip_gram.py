...
class SkipGram:
    """
    Skip-gram: given a center word, predict each surrounding context word.
    One NegativeSamplingLoss per context position; losses are summed.
    """
    ...
    def forward(self, contexts, target):
        # target: (N,)  — center word ids
        # contexts: (N, 2*window) — context word ids
...
if __name__ == "__main__":
    ...
    model     = SkipGram(vocab_size, hidden_size, window, corpus)
    optimizer = Adam()
    ...
    most_similar("goodbye", word_to_id, id_to_word, model.word_vecs)
